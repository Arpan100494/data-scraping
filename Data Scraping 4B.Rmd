...
conv_null = function(x){
  return (ifelse(is.null(x), 'Unknown', x))
}


for (repo in repos){
  curr_row = list(id=conv_null(repo$id),
                  name=conv_null(repo$name),
                  stars=conv_null(repo$stargazers_count),
                  created_at=conv_null(repo$created_at),
                  language=conv_null(repo$language),
                  description=conv_null(repo$description))
  print(curr_row)
